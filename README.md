...
1. 保留顶部搜索框专用翻译与展开保护，补充搜索组件回归校验
1. Chrome、Edge、Firefox 三端同步版本并重新生成发布压缩包

### v2.5.2 (2026-09-02)

1. 同步上游 Copilot 用量、复刻错误和仓库安全设置相关词条
1. 三端同步版本并重新生成发布压缩包

### v2.5.1 (2026-08-28)

1. 修复编辑区域保护规则误跳过 `input/textarea`，确保顶部搜索框占位符翻译与展开行为不受影响
1. 三端同步版本并重新生成发布压缩包
...
1. 修复更新提示横幅的关闭按钮点击无响应：横幅样式中的 `display: flex` 会覆盖浏览器默认的 `[hidden]` 规则，导致没有更新记录时横幅仍常驻显示，且关闭事件未绑定
1. 移除弹窗底部的「官网 · 更新日志」按钮，官网入口保留在更新提示横幅与插件主页链接中
1. 重新打包 Chrome、Edge、Firefox 三端发布压缩包
...
1. 改进时间元素处理：移除 `on` 后不再残留多余空格，并避免赋值时抹掉同级节点
