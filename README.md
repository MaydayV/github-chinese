...
### Chrome 应用商店安装与使用流程（推荐）

1. 打开安装地址（原链接）：https://chromewebstore.google.com/detail/emoeojemgbjcogiodobkpeohoailphgg?utm_source=item-share-cb
1. 点击进入安装页：[https://chromewebstore.google.com/detail/emoeojemgbjcogiodobkpeohoailphgg?utm_source=item-share-cb](https://chromewebstore.google.com/detail/emoeojemgbjcogiodobkpeohoailphgg?utm_source=item-share-cb)
1. 在 Chrome 商店页面点击 `添加至 Chrome`，并确认安装
1. 安装后可在 `chrome://extensions/` 中确认扩展已启用（建议固定到工具栏）
1. 打开任意 GitHub 页面即可自动中文化；如未立即生效，请刷新页面或重启浏览器

### Chrome 独立扩展版（无需用户脚本管理器）

1. 打开 Chrome，进入 `chrome://extensions/`。
1. 开启右上角 `开发者模式`。
1. 点击 `加载已解压的扩展程序`，选择当前仓库下的 `chrome` 目录（`/Users/colin/开发/谷歌插件/github-chinese-gh-pages/chrome`，包含 `manifest.json`）。
...
本项目词条仓库的原作者已将相关内容开源发布，因此本插件也继续以开源方式维护。
